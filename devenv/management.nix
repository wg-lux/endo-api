...
in
{
  ...
  scripts = {
    ...
    "docker-dev-build".exec = ''
      ...
      $RUNTIME build -f container/Dockerfile.dev -t endo-api:dev .
    '';
    ...
    "docker-dev-run".exec = ''
      ...
      RUNTIME=""
      if command -v podman >/dev/null 2>&1; then RUNTIME=podman; elif command -v docker >/dev/null 2>&1; then RUNTIME=docker; else echo "No container engine"; exit 1; fi
      $RUNTIME rm -f endo-api-dev >/dev/null 2>&1 || true
      $RUNTIME run -d --name endo-api-dev \
    ...
    "docker-prod-build".exec = ''
      ...
      RUNTIME=""
      if command -v podman >/dev/null 2>&1; then RUNTIME=podman; elif command -v docker >/dev/null 2>&1; then RUNTIME=docker; else echo "No container engine"; exit 1; fi
      $RUNTIME build -f container/Dockerfile.prod -t endo-api:prod .
    '';
    ...
    "docker-prod-run".exec = ''
      ...
      $RUNTIME rm -f endo-api-prod >/dev/null 2>&1 || true
      $RUNTIME run -d --name endo-api-prod \
        -p ${appConfig.server.port}:${appConfig.server.port} \
    ...
    "docker-logs".exec = ''
      ...
      tier="''${1:-dev}"; name="endo-api-$tier"; $RUNTIME logs -f "$name"
    '';

    "docker-stop".exec = ''
      RUNTIME=""; if command -v podman >/dev/null 2>&1; then RUNTIME=podman; elif command -v docker >/dev/null 2>&1; then RUNTIME=docker; else echo "No container engine"; exit 1; fi
      for tier in dev prod; do $RUNTIME rm -f endo-api-$tier >/dev/null 2>&1 || true; done
    '';

    "docker-clean".exec = ''
      RUNTIME=""; if command -v podman >/dev/null 2>&1; then RUNTIME=podman; elif command -v docker >/dev/null 2>&1; then RUNTIME=docker; else echo "No container engine"; exit 1; fi
      for img in endo-api:dev endo-api:prod; do $RUNTIME rmi "$img" >/dev/null 2>&1 || true; done
    '';
  };
}
